...
fetch('https://special-agreement-447fb99dc0.strapiapp.com/api/treinos?populate=*')
  ...
  .then(data => {
    ...
    select.addEventListener('change', function () {
      ...
      fetch(`https://special-agreement-447fb99dc0.strapiapp.com/api/treinos/${selectedValue}?populate=*`)
        .then(response => response.json())
        .then(data => {
          const newEmployees = data.data.attributes.employees.data.map(employee => ({
          ...
          fetch(`http://localhost:8080/${selectedValue}`)
            ...
            .then(localData => {
              ...
              localEmployees.forEach(localEmp => {
                const existingEmpIndex = newEmployees.findIndex(newEmp => newEmp.employee_id === localEmp.employee_id.toString());
        ...
        .catch(error => console.error('Erro ao carregar os dados dos empregados:', error));
    });
  })
